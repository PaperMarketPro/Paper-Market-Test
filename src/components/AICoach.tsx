...
export const AICoach: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'teach' | 'scorecard' | 'train'>('chat');
  ...
  return (
    <div className="space-y-6 pb-24 max-w-4xl mx-auto w-full">
      ...
      <AnimatePresence mode="wait">
        ...
        {/* Tab 4: AI LLM Training & Parameters Tuning */}
        {activeTab === 'train' && (
          <motion.div
            key="train"
            initial={{ opacity: 0, y: 10 }}
          ...
          >
            ...
            <div className="bg-[#11141c] border border-white/5 rounded-2xl p-5 relative overflow-hidden shadow-xl">
              ...
              <h3 className="text-base font-bold text-white mb-1.5">Train AI Trading Coach</h3>
              <p className="text-xs text-gray-400 leading-relaxed max-w-2xl font-sans">
                Tweak hyper-parameters, prime neural personas, and execute specialized alignment pipelines. 
                Running a training cycle indexes your historical trade ledger and custom cognitive rules directly into the Gemini model's contextual memory, generating highly accurate and personalized risk advice.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="bg-[#11141c] border border-white/5 rounded-2xl p-5 space-y-5 shadow-xl">
                <div className="flex items-center gap-1.5 text-xs text-sky-400 font-mono uppercase font-bold tracking-wider">
                  <Sliders className="w-4 h-4" /> Model Hyperparameters
                </div>

                {/* Model Selector */}
                <div className="space-y-2">
                  <label className="text-[10px] text-gray-400 font-mono uppercase block">Selected LLM Base Model</label>
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      ...
                      className={`p-3 rounded-xl border text-left flex flex-col justify-between transition cursor-pointer h-24 ${
                        selectedModel === 'gemini-3.5-flash'
                          ...
                          : 'border-white/5 bg-[#171b26]/50 text-gray-400 hover:border-white/10 hover:bg-[#171b26]'
                      }`}
                    >
                      <div className="flex justify-between items-center w-full">
                        <span className="text-xs font-bold">Gemini 3.5 Flash</span>
                        {selectedModel === 'gemini-3.5-flash' && <Check className="w-3.5 h-3.5 text-sky-400" />}
                      </div>
                      <span className="text-[9px] text-gray-500 font-sans leading-normal">
                        Optimized for fast CBT conversations and real-time trade execution reviews.
                      </span>
                    </button>
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left flex flex-col justify-between transition cursor-pointer h-24 relative ${
                    ...
                    >
                      <div className="flex justify-between items-center w-full">
                        ...
                        {selectedModel === 'gemini-3.1-pro-preview' && <Check className="w-3.5 h-3.5 text-sky-400" />}
                      </div>
                      <span className="text-[9px] text-gray-500 font-sans leading-normal">
                        Advanced reasoning model for complex option patterns and multi-session psychometrics.
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  <div className="flex justify-between items-center text-[10px] font-mono text-gray-400 uppercase">
                    ...
                    <span className="text-sky-400 font-bold tabular-numbers">{temperature.toFixed(1)}</span>
                  </div>
                  <input
                    type="range"
                    min="0.1"
                    ...
                    className="w-full h-1 bg-[#171b26] rounded-lg appearance-none cursor-pointer accent-sky-500"
                  />
                  <div className="bg-[#171b26]/40 rounded-xl p-2.5 border border-white/5">
                    <span className="text-[10px] text-gray-400 leading-relaxed font-sans block">
                      {temperature < 0.3 ? (
                        <strong>Deterministic (Strict Enforcer):</strong>
                      ) : temperature <= 0.7 ? (
                        <strong>Balanced Mentor (Optimal Coaching):</strong>
                      ) : (
                        <strong>Creative Spec (Abstract Thinker):</strong>
                      )}
                      {" "}
                      {temperature < 0.3 
                        ? "AI enforces tight risk parameters, clinical If-Then rules, and direct literal setups with minimal creative padding." 
                        : temperature <= 0.7 
                        ? "AI unifies trading mathematics with cognitive psychology, adjusting dynamically to emotional journals." 
                        : "AI provides abstract risk correlations, exploratory trade theories, and broader market hypothesis."}
                    </span>
                  </div>
                </div>

                {/* Safety Rule Injection */}
                <div className="flex items-center justify-between p-3 bg-[#171b26]/40 border border-white/5 rounded-xl">
                  <div className="space-y-0.5 max-w-[80%]">
                    <span className="text-xs font-bold text-white block">Inject Custom Cognitive Rules</span>
                    <span className="text-[10px] text-gray-500 leading-normal block">
                      Hardcode your active If-Then rules (e.g. locks, breathing triggers) as strict model constraints.
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setInjectCognitiveRules(!injectCognitiveRules)}
                    className={`w-9 h-5 rounded-full p-0.5 transition-colors duration-200 focus:outline-none cursor-pointer ${
                      injectCognitiveRules ? 'bg-sky-500' : 'bg-white/10'
                    }`}
                  >
                    <div className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-200 ${
                      injectCognitiveRules ? 'translate-x-4' : 'translate-x-0'
                    }`} />
                  </button>
                </div>
              </div>

              {/* Right Column: Persona & Grounding */}
              <div className="bg-[#11141c] border border-white/5 rounded-2xl p-5 space-y-5 shadow-xl">
                <div className="flex items-center gap-1.5 text-xs text-sky-400 font-mono uppercase font-bold tracking-wider">
                  <Award className="w-4 h-4" /> Neural Persona Alignment
                </div>

                {/* Personas Selector */}
                <div className="space-y-2">
                  <label className="text-[10px] text-gray-400 font-mono uppercase block">Active Coaching Voice</label>
                  <div className="grid grid-cols-3 gap-2">
                    {(['Market Veteran', 'Quantitative Analyst', 'Clinical Psychologist'] as const).map(p => (
                      <button
                        key={p}
                        type="button"
                        onClick={() => setSystemPersona(p)}
                        className={`p-2.5 rounded-xl border text-center flex flex-col items-center justify-center transition cursor-pointer gap-1.5 ${
                          systemPersona === p
                            ? 'border-sky-500 bg-sky-500/5 text-white font-bold'
                            : 'border-white/5 bg-[#171b26]/50 text-gray-400 hover:border-white/10 hover:bg-[#171b26]'
                        }`}
                      >
                        <span className="text-[10px] block leading-tight">{p}</span>
                      </button>
                    ))}
                  </div>
                </div>

                {/* Custom Grounding Textarea */}
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] text-gray-400 font-mono uppercase">Custom Training Guidelines</label>
                    <span className="text-[9px] text-gray-500">Grounded prompts</span>
                  </div>
                  <textarea
                    rows={4}
                    value={customGrounding}
                    onChange={(e) => setCustomGrounding(e.target.value)}
                    placeholder="Enter custom guidelines, specific strategies (e.g. support/resistance), lot restrictions, or specific stocks to watch..."
                    className="w-full bg-[#171b26] border border-white/5 text-xs text-white rounded-xl p-3 focus:outline-none focus:ring-1 focus:ring-sky-500 placeholder-gray-600 font-sans resize-none"
                  />
                  
                  {/* Preset Grounding Shortcuts */}
                  <div className="space-y-1.5">
                    <span className="text-[8px] font-mono text-gray-500 uppercase tracking-widest block">Prefill guidelines templates:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {[
                        { 
                          label: "Day Trading", 
                          text: "Focus heavily on price action, ignore pre-market noise, enforce exit of all positions before market close at 3:30 PM." 
                        },
                        { 
                          label: "Options Scalper", 
                          text: "Strict focus on high IV option premiums. Force stop-loss exit if option value falls below 15% of entry price. Keep sizing under 2 lots." 
                        },
                        { 
                          label: "Swing Risk", 
                          text: "Analyze multi-day EMA supports. Prioritize holding winners for at least 3-5 days. Always maintain a minimum 1:2 risk-to-reward ratio." 
                        }
                      ].map((preset, idx) => (
                        <button
                          key={idx}
                          type="button"
                          onClick={() => setCustomGrounding(preset.text)}
                          className="bg-white/3 hover:bg-white/6 border border-white/5 px-2 py-1 rounded-lg text-[9px] text-gray-300 transition cursor-pointer"
                        >
                          + {preset.label}
                        </button>
                      ))}
            ...
            <div className="bg-[#11141c] border border-white/5 rounded-2xl p-5 space-y-4 shadow-xl">
              <div className="flex justify-between items-center">
                <div className="space-y-0.5">
                  <span className="text-[9px] font-mono text-sky-400 uppercase font-bold tracking-widest block">
                    Execution Engine
                  </span>
                  <h4 className="text-sm font-bold text-white">Active Alignment Pipeline</h4>
                </div>
                
                {/* Datasets counters */}
                <div className="flex gap-3 text-right">
                  <div className="bg-[#171b26] border border-white/5 px-2.5 py-1 rounded-xl">
                    <span className="text-[8px] font-mono text-gray-500 uppercase block">Ledger Logs</span>
                    <span className="text-[11px] font-bold text-white font-mono">{positions.length}</span>
                  </div>
                  <div className="bg-[#171b26] border border-white/5 px-2.5 py-1 rounded-xl">
                    <span className="text-[8px] font-mono text-gray-500 uppercase block">Journal Entries</span>
                    <span className="text-[11px] font-bold text-white font-mono">{journals.length}</span>
                  </div>
                  <div className="bg-[#171b26] border border-white/5 px-2.5 py-1 rounded-xl">
                    <span className="text-[8px] font-mono text-gray-500 uppercase block">Active Rules</span>
                    <span className="text-[11px] font-bold text-white font-mono">{cognitiveRules.filter(r => r.isActive).length}</span>
                  </div>
                </div>
              </div>
              ...
              {isTrainingRunning && (
                <div className="space-y-2.5">
                  <div className="flex justify-between items-center text-[10px] font-mono text-sky-400 animate-pulse">
                    <span className="uppercase font-bold">PIPELINE RUNNING: {trainingStatus}</span>
                    <span>{trainingProgress}%</span>
                  </div>
                  <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                    <div 
                      className="bg-sky-500 h-full transition-all duration-300 shadow-lg shadow-sky-500/20" 
              ...
              {(trainingLogs.length > 0 || isTrainingRunning) && (
                <div className="bg-black/45 rounded-xl border border-white/5 p-3.5 font-mono text-[10px] text-gray-400 space-y-1 max-h-[140px] overflow-y-auto shadow-inner">
                  ...
                  {isTrainingRunning && (
                    <div className="text-sky-400 font-bold animate-pulse flex items-center gap-1.5 pt-1.5">
                      <RefreshCw className="w-3 h-3 animate-spin shrink-0" />
                      <span>Optimizing contextual embeddings...</span>
                    </div>
                  )}
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-sky-500 hover:bg-sky-600 disabled:bg-[#171b26] disabled:border-white/5 disabled:text-gray-500 text-white font-bold py-3.5 rounded-xl text-xs flex items-center justify-center gap-1.5 transition cursor-pointer shadow-lg shadow-sky-500/15 border border-transparent"
              >
                {isTrainingRunning ? (
                  <>
                    <RefreshCw className="w-4 h-4 animate-spin" />
                    Fine-Tuning AI Model Weights...
                  </>
                ) : (
                  <>
                    <Activity className="w-4 h-4" />
                    Save & Train AI Coach Model
                  </>
                )}
              </button>
              
              {/* Success Tip block */}
              {!isTrainingRunning && trainingProgress === 100 && (
                <motion.div
                  initial={{ opacity: 0, y: 5 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3 flex items-start gap-2 text-[10px] text-emerald-400"
                >
                  <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                  <div className="space-y-0.5">
                    <span className="font-bold block">AI Trained successfully!</span>
                    <p className="text-gray-400 leading-normal">
                      The AI Trading psychologist model has been updated with your new hyperparameters, active safety rules, and custom grounding guidelines. Future chats and course syllabi will prioritize these parameters.
                    </p>
                  </div>
                </motion.div>
              )}
