...
import { 
  ...
  Activity, HelpCircle, ArrowRight, Shield, RefreshCw, Search, ShieldCheck 
} from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  const { user, positions, instruments, toggleTheme, theme } = useApp();
  if (!user) return null;
  ...
  return (
    <div className="space-y-5 pb-24 max-w-md mx-auto">
      {/* BRAND HEADER HERO */}
      <div className="flex justify-between items-center bg-[#0c1020]/45 border border-white/5 rounded-2xl px-4 py-3.5 shadow-lg backdrop-blur-sm">
        <BrandLogo size="md" />
        <div className="flex items-center gap-1.5 bg-amber-500/10 px-2.5 py-1.5 rounded-xl border border-amber-500/15 text-[10px] font-bold text-amber-500 font-mono">
          🔥 {user.streak} Days
        </div>
      </div>
      ...
      <div className="relative font-sans">
        ...
        <input
          ...
          className="w-full bg-[#0a0d16] border border-white/5 focus:border-sky-500/20 rounded-2xl pl-10 pr-4 py-3.5 text-xs text-gray-300 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-sky-500/25 transition duration-200"
